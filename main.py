...
def generate_preview_clip(video_file: str, output_dir: str, clip_count=10, clip_duration=3,
    ...
    video_codec = "h264_qsv" if hw_accel == "qsv" else "libx264"
    for i in tqdm(range(clip_count)):
        ...
        subprocess.run(["ffmpeg", "-i", video_file, "-ss", str(clip_start), "-t", str(clip_duration), "-vf",
            f"scale={output_resolution[0]}:{output_resolution[1]}", "-c:v", video_codec, "-c:a", "aac", output_file])
    ...
    subprocess.run(
        ["ffmpeg", "-f", "concat", "-safe", "0", "-i", os.path.join(output_dir, "concat.txt"), "-c:v", video_codec,
         "-c:a", "copy", os.path.join(output_dir, "preview.mp4")])
    for i in range(clip_count):
